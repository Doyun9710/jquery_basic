	...
	$( document ).ready( function() {
		...
		$( document ).on( 'click', 'button.action', function() {
			if( $( this ).attr( 'action' ) == 'write' ) {
				...
				const mail = $(this).attr('mail');
				const arrmail = mail.split( '@' );
				const mail1 = arrmail[0];
				const mail2 = arrmail[1];
		...
		$.ajax({
			...
			success: function( jsonData ) {
				...
				$.each( jsonData, function(index, item) {
					let html = '';
					if( item.wgap < 1 ) {
						html = '<h3>' + item.seq + ' : ' + item.writer + '(' + item.hit + ') new</h3>';
					} else {
						html = '<h3>' + item.seq + ' : ' + item.writer + '(' + item.hit + ')</h3>';
					}
					html += '<div>';
